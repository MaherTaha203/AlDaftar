 ...
 *
 * Motion: the carve GLIDES between sections via the shared `useNavGlide`
 * (View Transitions; see nav-glide.ts). Density: item height tracks
 * `--ctrl-h`, and the rail is condensed to fit common viewports without
 * scrolling — a scrollbar would sit on the seam and sever the carve
 * (see `.rail-scroll`).
 *
 * Business-blind: navigation groups/items arrive as props. Between lg and xl
 * the rail collapses to icons (the carve still works — it is layout, not
 * decoration); below md the shell hides the rail entirely and renders the
 * BottomNav instead (the phone face of the same solid).
 */
export interface SidebarItem {
  label: string;
  href: string;
  icon?: ReactNode;
  /** Promoted to the phone bottom bar (see BottomNav); others fold politely. */
...
export function Sidebar({ groups, brand, className }: SidebarProps) {
  ...
  return (
    ...
    >
      {brand ? <div className="px-md max-xl:px-xs">{brand}</div> : null}

      {groups.map((group) => (
        <div key={group.label} className="flex flex-col gap-xs">
          <p className="px-md text-[11px] font-medium tracking-wide text-white/40 max-xl:sr-only">
            {group.label}
          </p>
