# BDD-001 — (title pending verbatim import)

> **Status: Approved externally — pending verbatim import.**
>
> This is a **placeholder only**. BDD-001 was approved outside this repository,
...
> with its verbatim text and rename it to `BDD-001-<Subject>.md`.

## What this placeholder is

- A marker that BDD-001 exists and is approved, so the gap is tracked rather
...
## Documents that reference BDD-001 (dependents)

These existing documents name BDD-001 and cannot be finalized _against_ it
...
- `business-architecture.md` §3 — open scope decisions **PD-01** (v1 module
  scope) and **PD-02** (ledger model) remain Pending; whether BDD-001 bears on
