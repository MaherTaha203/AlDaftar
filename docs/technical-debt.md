...
| TD-004 | **REMOVED 2026-07-05 (DL-031).** Persistence now runs on Supabase (records via `SupabaseRecordStore`, binaries via the `attachments` Storage bucket), registered at the composition root. The local adapters remain only as the no-provider default (tests) and as the read source of the one-time `window.aldaftarMigrateLocalData()` import. | —                                                                                                                                                                                                                    | —                                                                                                                                                                                                                          | —        | Done — see DL-031.                                                                                                                                                                                                                                                                                                                             |
...
- **Cross-module read models are delivered** via `lib/modules/reporting`
  ...
  delete/post across the write paths, and — since DL-032 — `Login`/`Logout`
  from the authentication flow. Only `Unpost` remains a reserved action with
  no producer (awaits BDR-07).
- **BDR-08 (attachments policy) interims — pending owner decision.** The
  ...
  is answered.
- **File storage is now Supabase Storage** (bucket `attachments`, DL-031):
  `getFileStore()` returns the Supabase implementation after the bootstrap
  registers it; the IndexedDB adapter remains only as the no-provider default
  and as the migration read source.
...
- **Backup & recovery mechanics — delivered (DL-034):** `npm run backup` /
  `npm run restore` + `docs/operations/BACKUP_AND_RECOVERY.md`. What remains is
  the owner's _policy_ (schedule, retention, managed-backup plan) + one DR
  rehearsal — operational, not code.
- **Consciously deferred under the single-owner model (Lead-Architect triage
  2026-07-05), not defects** — each addresses multi-user/high-concurrency scale
  a single owner never reaches; safe today, recorded so a future need is not
  lost: (1) transactional DB numbering sequence — the partial-unique `number`
  index blocks duplicates and paginated `findAll` makes `nextNumber` see the
  true max; (2) DB guard against over-return under concurrency — needs two
  simultaneous posts; (3) foreign-key constraints — low risk under
  archive-not-delete, safe add needs a data pass; (4) migration runner +
  version table — four hand-applied files with `verify:schema` suffice for now.
  Promote order if ever needed: (3) → (4) → (1) → (2).
- **Follow-up when Supabase lands:** reconcile the infrastructure
  repository/storage
